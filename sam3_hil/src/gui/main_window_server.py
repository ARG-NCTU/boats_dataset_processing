...
class STAMPMainWindow(QMainWindow):
    ...
    def __init__(self, engine=None, mode: str = "local", server_url: str = None):
        super().__init__()
        
        # === 新增：Server 模式設定 ===
        self._execution_mode = mode  # "local" 或 "server"
        self._server_url = server_url
        self._external_engine = engine  # 外部傳入的 engine
        ...
        self.sam3_engine = self._external_engine  # Reuse for refinement
    ...
    def _run_video_detection(self, prompt: str, mode: str):
        ...
        if self._execution_mode == "server":
            # Server 模式：使用 ServerVideoDetectionWorker
            from src.gui.server_workers import ServerVideoDetectionWorker
            self.worker = ServerVideoDetectionWorker(
                server_url=self._server_url,
                video_path=video_path,
                prompt=prompt,
            )
        else:
            # 本地模式：使用原本的 SAM3Worker
    ...
    def _run_batch_detection(self, prompt: str, mode: str):
        ...
        if self._execution_mode == "server":
            # Server 模式：使用 ServerBatchDetectionWorker
            from src.gui.server_workers import ServerBatchDetectionWorker
            self.worker = ServerBatchDetectionWorker(
                server_url=self._server_url,
                image_paths=image_paths,
                prompt=prompt,
            )
        else:
            # 本地模式：使用原本的 ImageBatchWorker
        ...
        if self.sam3_engine is None:
            try:
                if self._execution_mode == "server":
                    ...
                    self.sam3_engine = StampAPIClient(self._server_url)
                else:
