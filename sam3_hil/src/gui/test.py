...
def estimate_vram_usage(total_frames: int, width: int, height: int) -> dict:
    """
    估算 SAM3 處理影片所需的 VRAM。
    
    基於實測數據：
    - 500 幀 @ 1920x480 ≈ 6.7 GB
    - 600 幀 @ 1920x1080 ≈ 22+ GB
    ...
    Returns:
        dict: {
            'estimated_gb': 估計 VRAM (GB),
            'recommended_frames': 建議最大幀數,
            'risk_level': 'safe' / 'warning' / 'danger'
        }
    """
    # 基準：500 幀 @ 1920x1080 ≈ 18 GB
    base_frames = 500
    base_resolution = 1920 * 1080
    base_vram = 18.0  # GB
    
    # 計算比例
    resolution_ratio = (width * height) / base_resolution
    frame_ratio = total_frames / base_frames
    
    # 估算 VRAM（線性近似）
    estimated_gb = base_vram * resolution_ratio * frame_ratio
    
    # 假設 24GB GPU，留 2GB 給系統
    available_vram = 22.0
    
    # 計算建議的最大幀數
    if resolution_ratio > 0:
        recommended_frames = int(base_frames * (available_vram / base_vram) / resolution_ratio)
    else:
        recommended_frames = 500
    
    # 風險等級
    if estimated_gb < available_vram * 0.7:
        risk_level = 'safe'
    elif estimated_gb < available_vram * 0.9:
        risk_level = 'warning'
    else:
        risk_level = 'danger'
...
class HILAAMainWindow(QMainWindow):
    ...
    def run_detection(self):
        ...
        logger.info(f"VRAM estimate: {vram_info['estimated_gb']:.1f} GB for "
                   f"{metadata.total_frames} frames @ {metadata.width}x{metadata.height}")
        
        if vram_info['risk_level'] == 'danger':
            # 高風險：顯示警告並詢問是否繼續
            reply = QMessageBox.warning(
                self,
                "⚠️ VRAM Warning - High Risk of OOM",
                f"Video specs may exceed GPU memory!\n\n"
                f"📹 Video: {metadata.total_frames} frames @ {metadata.width}x{metadata.height}\n"
                f"💾 Estimated VRAM: {vram_info['estimated_gb']:.1f} GB\n"
                f"🎯 Available VRAM: ~{vram_info['available_vram']:.0f} GB\n\n"
                f"⚡ Recommended max frames: {vram_info['recommended_frames']}\n\n"
                f"Options:\n"
                f"• Click 'Yes' to try anyway (may crash)\n"
                f"• Click 'No' to cancel and trim your video:\n"
                f"  ffmpeg -i input.mp4 -vframes {vram_info['recommended_frames']} output.mp4",
        ...
        elif vram_info['risk_level'] == 'warning':
            # 中等風險：顯示提示
            logger.warning(f"VRAM usage may be high: {vram_info['estimated_gb']:.1f} GB estimated")
